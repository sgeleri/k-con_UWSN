...
def _ordered_path(
    ...
    """Order a solved non-bifurcating arc set from source to BS."""
    ...
    if remaining:
        raise ValueError(
            f"active path from source {source} contains disconnected arcs"
        )
...
def _status_only_solution(problem: pulp.LpProblem) -> Solution:
    return Solution(
...
def extract_solution(
    ...
    *,
    ...
    active_arcs = {
        key: 1
        for key, variable in variables.arc_used.items()
        if _value(variable) > 0.5
    }
    paths = _reconstruct_paths(env, active_arcs, path_packets)
